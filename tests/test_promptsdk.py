...
mocked_api.invoke.return_value = (
    None,
...
class TestPromptSDK(unittest.TestCase):
    def test_uses_correct_endpoint(self):
        prompt = PromptSDK(
            mocked_api, cache=mocked_cache, fallback_cache=fallback_cache, logger=logger
        )
    ...
    def test_passes_correct_dto(self, slug, version, tag):
        ...
        self.assertEqual(dto, GetPromptDTO(slug=slug, version=version, tag=tag))
    ...
    def test_replaces_variables(self):
        ...
        _, prompt_response, generation = prompt.get(
            "slug", variables={"name": "Basalt"}
        )
    ...
    def test_returns_generation_object(self):
        ...
        _, _, generation = prompt.get(
            "test-slug", version="1.0", tag="prod", variables={"key": "value"}
        )
