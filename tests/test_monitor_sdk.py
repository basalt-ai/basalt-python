...
class TestMonitorSDK(unittest.TestCase):
    ...
    def test_create_generation(self):
        ...
        log = trace.create_log(
            {"type": "span", "name": "test-log", "input": self.content}
        )
    ...
    def test_update_generation(self):
        ...
        generation = log.create_generation(
            {"name": "test-generation", "input": self.content}
        )
        ...
        self.assertEqual(
            generation.metadata, {"updated": True, "timestamp": "2023-01-01"}
        )
    ...
    def test_nested_logs(self):
        ...
        parent_log = trace.create_log(
            {"type": "span", "name": "parent-log", "input": self.content}
        )

        child_log = parent_log.create_log(
            {"type": "span", "name": "child-log", "input": "Child input"}
        )
    ...
    def test_trace_identify(self):
        ...
        # Identify the trace
        trace.identify(
            {"user": self.user, "organization": {"id": "org-123", "name": "Basalt"}}
        )
...
class TestMonitorSDKIntegration(unittest.TestCase):
    ...
    def test_complex_workflow(self):
        ...
        main_trace = self.monitor.create_trace(
            ...
            {"input": self.query, "user": self.user, "name": "Complex Workflow Test"},
        )

        # Step 1: Content generation
        generation_span = main_trace.create_log(
            {"type": "span", "name": "content-generation", "input": self.query}
        )
        ...
        # Get prompt from Basalt
        err, prompt_response, generation = prompt_sdk.get(
            "generate-content", variables={"query": self.query}, version="1.0"
        )
        ...
        generation_log.update(
            {"output": generated_text, "metadata": {"processingTime": 500}}
        )
        ...
        classification_span = main_trace.create_log(
            {"type": "span", "name": "classification", "input": generated_text}
        )
        ...
        err, classify_prompt_response, classify_generation = prompt_sdk.get(
            "classify-content", variables={"content": generated_text}, version="1.0"
        )
        ...
        # Update generation with output
        class_gen.update({"output": categories})
