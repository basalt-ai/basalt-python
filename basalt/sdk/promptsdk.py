...
class PromptSDK:
    ...
    def __init__(
        self, api: IApi, cache: ICache, fallback_cache: ICache, logger: ILogger
    ):
        self._api = api
        self._cache = cache
    ...
    def get(
        ...
        """
        dto = GetPromptDTO(slug=slug, version=version, tag=tag)
        ...
        if cached:
            ...
            generation = self._prepare_monitoring(
                prompt_response, slug, version, tag, variables, original_prompt_text
            )
        ...
        generation = Generation(
            {
                "name": slug,
                "trace": trace,
                "prompt": {"slug": slug, "version": version, "tag": tag},
                "input": original_prompt_text or prompt.text,
                "variables": variables,
                "options": {"type": "single"},
            }
    ...
    def describe(
        ...
        """
        dto = DescribePromptDTO(slug=slug, version=version, tag=tag)
