...
def replace_variables(template: str, replacements: dict) -> Tuple[Set[str], str]:
    ...
    replacer = (
        lambda match: str(get(match.group(1)))
        if get(match.group(1))
        else match.group(0)
    )

    replaced = reg_replace(r"{{(.*?)}}", replacer, template)
...
def pick_number(dict: Dict[str, Any], field_name: str) -> float:
    ...
    raise TypeError(
        f"Field '{field_name}' must be a number (int or float), got {type(value).__name__}."
    )
