...
class SendTraceEndpoint:
    ...
    def prepare_request(self, dto: Optional[Input] = None) -> Dict[str, Any]:
        ...
        if not dto or "trace" not in dto:
            return {"method": "post", "path": "/monitor/trace", "body": {}}
        ...
        if isinstance(trace, dict):
            ...
            for log in trace_data["logs"]:
                ...
                log_data["startTime"] = (
                    log_data["start_time"].isoformat()
                    if isinstance(log_data["start_time"], datetime)
                    else log_data["start_time"]
                )
                log_data["endTime"] = (
                    log_data["end_time"].isoformat()
                    if isinstance(log_data["end_time"], datetime)
                    and log_data["end_time"]
                    else None
                )
                ...
                if hasattr(log, "variables") and log.variables:
                    log_data["variables"] = [
                        ...
                        for key, value in log.variables.items()
                    ]
        ...
        for log_data in logs:
            ...
            if "startTime" in log_data and "endTime" in log_data:
                ...
                if "start_time" in processed_log:
                    processed_log["startTime"] = (
                        processed_log["start_time"].isoformat()
                        if isinstance(processed_log["start_time"], datetime)
                        else processed_log["start_time"]
                    )
                    del processed_log["start_time"]
                if "end_time" in processed_log:
                    processed_log["endTime"] = (
                        processed_log["end_time"].isoformat()
                        if isinstance(processed_log["end_time"], datetime)
                        and processed_log["end_time"]
                        else None
                    )
        ...
        return {"method": "post", "path": "/monitor/trace", "body": body}
